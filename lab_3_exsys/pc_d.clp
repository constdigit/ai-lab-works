...
=> (printout t
	...
	(printout t "  - Возможно, ваша материнская плата не работает в двухканальном
	режиме с данной RAM." crlf)
	(printout t "  - Свяжитесь с тех. поддержкой производителя." crlf)
)
...
=> (printout t
	...
	(printout t "  - Проверьте выставленное в BIOS значение частоты и действующую
	частоту RAM (на задней крышке упаковки)." crlf)
...
=> (printout t
	...
	(printout t "  - В настройках BIOS выставьте соотвествующую частоту для
	корректной работы с другими элементами." crlf)
...
=> (printout t
		 "-----------------------------------------------------------------------------
	--------------------------------------------" crlf)
	(printout t "Если материнская плата не видит контроллер, подключенный
	через PCI:" crlf)
...
=> (printout t
	...
	(printout t "  - Проверьте периферийные устройства на другом компьютере,
	возможно, они неисправны." crlf)
...
(defrule hdd-power
	...
	=> (printout t "Проверьте подключение жесткого диска, проверьте кабель
	SATA." crlf)
...
(defrule hdd-badblocks
	...
	=> (printout t
		 "-----------------------------------------------------------------------------
	--------------------------------------------" crlf)
	(printout t "Проведите проверку количество \"плохих\" блоков в жестком диске
	при помощи одной из доступных утилит." crlf)
...
(defrule hdd-defect
  ...
  => (printout t
	...
	(printout t "Обратитесь в сервисный центр или место приобретения для ремонта
	или замены жесткого диска по гарантии." crlf)
	)
...
(defrule hdd-badsysfiles
	...
	=> (printout t
		     "-----------------------------------------------------------------------------
	--------------------------------------------" crlf)
	(printout t "Запустите вашу операционную систему в безопасном режиме
	(safe mode) или в режиме восстановления." crlf)
	(printout t "Операционная система должна самостоятельно решить проблему с её
	системными файлами. Возможно потребуется переустановка ОС." crlf)
	(printout t "Не выключайте больеш компьютер иначе как через команду \"Выключение\"
	в операционной системе." crlf)
	)
...
(defrule hdd-overheating
	...
	=> (printout t
		     "-----------------------------------------------------------------------------
	--------------------------------------------" crlf)
	(printout t "Очистите ваш жесткий диск от пыли. Убедитесь, что вокруг него
	достаточно свободного пространства, что обеспечивается беспрепятственный поток
	воздуха." crlf)
...
(defrule hdd-replacement
  ...
  => (printout t
	...
	--------------------------------------------" crlf)
  (printout t "Скорее всего, ваш жесткий диск неисправен." crlf)
  (printout t "Обратитесь к специалисту для восстановления данных, и приобретите
	новый жесткий диск." crlf)
)
...
(defrule hdd-is-okay
  ...
  (printout t "Рекомендуется регулярно делать бекап (резервное сохранение данных)
	и проверять количество \"плохих\" блоков (bad blocks). Если их число начнёт
	резко расти, следует заменить жесткий диск." crlf)
)
